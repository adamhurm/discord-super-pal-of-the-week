...
async def collection_trade_in(
    ...
    received = (
        random.choice(eligible) if eligible else {"display_name": "Unknown", "avatar_url": None}
    )
    ...
    existing = next(
        (
            c
            for c in MOCK_OWNED
            if c["member_id"] == received["discord_id"] and c["rarity"] == rarity
        ),
        None,
    )
    if existing:
