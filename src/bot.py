...
@app_commands.describe(amount="Pringles to spend (200 Pringles → 100 Palycoins)")
async def palymarket_exchange(interaction: discord.Interaction, amount: int) -> None:
    await interaction.response.defer(ephemeral=True)
    success, reason = await palymarket_svc.exchange_pringles(str(interaction.user.id), amount)
    if success:
        await interaction.followup.send(
            f"Exchanged {amount} Pringles for {(amount // 200) * 100} Palycoins!",
            ephemeral=True,
        )
    elif reason == "minimum_not_met":
        await interaction.followup.send(
            "Must exchange at least 200 Pringles at a time.", ephemeral=True
        )
    else:
